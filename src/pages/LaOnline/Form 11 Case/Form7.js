...
const Form7 = ({ idForm, closeCanvas }) => {
  ...
  const formValidate = useFormik({
    ...
    validationSchema: Yup.object().shape({
      leave_name: Yup.string().required("กรุณาระบุข้อมูล"),
  ...
  const handleSubmit = async () => {
    try {
      await axios.post(`http://localhost:8000/uploadform`, {
        ...
        trainingFundFrom: data.trainingFundFrom,
  ...
  return (
    ...
    >
      ...
      <FormGroup row>
        <Label className="text-end" xl={2}>
          <span className="me-1 text-danger">*</span>เรื่อง
        </Label>
        <Col xl={4}>
          {/* <Input value={data.askFor !== "" ? data.askFor : "กรุณาระบุ"} onChange={(e) => handleInputChange(e, 'title')} /> */}
          <Input
            name="leave_name"
            value={formValidate.values.leave_name}
            onBlur={formValidate.handleBlur}
            onChange={(e) => {
              handleInputChange(e, "leave_name")
              formValidate.handleChange(e)
            }}
            invalid={formValidate.touched.leave_name && formValidate.errors.leave_name ? true : false}
            valid={formValidate.touched.leave_name && !formValidate.errors.leave_name ? true : false}
          />
          {formValidate.touched.leave_name && formValidate.errors.leave_name ? 
          <FormFeedback type="invalid">{formValidate.errors.leave_name}</FormFeedback> : 
      ...
      <FormGroup row>
        ...
        <Col xl={4}>
          <Input type="text" onChange={(e) => handleInputChange(e, 'trainingFundFrom')} />
        </Col>
      </FormGroup>
      <FormGroup row>
