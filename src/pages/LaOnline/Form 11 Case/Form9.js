...
const Form9 = ({ idForm, closeCanvas }) => {
  ...
  const formValidate = useFormik({
    ...
    validationSchema: Yup.object().shape({
      outside_place: Yup.string().required("กรุณาระบุข้อมูล"),
  ...
  const [data, setData] = useState({
    ...
    title: "การไปปฏิบัติราชการนอกสำนักงาน ก.พ.ร.",
  ...
  const handleSubmit = async () => {
    try {
      await axios.post(`http://localhost:8000/uploadform`, {
        ...
        outsidePlace: data.outsidePlace,
  ...
  return (
    ...
    >
      ...
      <FormGroup row>
        <Label className="text-end" xl={2}>
        <span className="me-1 text-danger">*</span>สถานที่
        </Label>
        <Col xl={4}>
          <Input 
          name="outside_place" 
          type="text" 
          value={formValidate.values.outside_place} 
          onBlur={formValidate.handleBlur}
          onChange={(e) => {
            handleInputChange(e, 'outsidePlace')
            formValidate.handleChange(e)
          }}
          invalid={formValidate.touched.outside_place && formValidate.errors.outside_place ? true : false}
          valid={formValidate.touched.outside_place && !formValidate.errors.outside_place ? true : false}
            />
            {formValidate.touched.outside_place && formValidate.errors.outside_place ? (
              <FormFeedback type="invalid">{formValidate.errors.outside_place}</FormFeedback>
