...
const Form6 = ({ idForm, closeCanvas }) => {
  ...
  const formValidate = useFormik({
    ...
    validationSchema: Yup.object().shape({
      conscription_from: Yup.string().required("กรุณาระบุข้อมูล"),
      conscription_place: Yup.string().required("กรุณาระบุข้อมูล"),
      conscription_date: Yup.string().required("กรุณาระบุข้อมูล"),
      conscription_for: Yup.string().required("กรุณาระบุข้อมูล"),
      conscription_to_place: Yup.string().required("กรุณาระบุข้อมูล"),
    }),
    onSubmit: (values) => {
      console.log(values);
    },
  ...
  const [data, setData] = useState({
    ...
    title: "ขอลาเพื่อเข้ารับการเตรียมพล",
  ...
  const handleSubmit = async () => {
    try {
      await axios.post(`http://localhost:8000/uploadform`, {
        ...
        conscriptFrom: data.conscriptFrom,
        conscriptPlace: data.conscriptPlace,
        conscriptFor: data.conscriptFor,
        conscriptToPlace: data.conscriptToPlace,
        conscriptDate: data.conscriptDate.toLocaleDateString(),
  ...
  return (
    ...
    >
      ...
      <FormGroup row>
        <Label className="text-end" xl={2}>
        <span className="me-1 text-danger">*</span>ได้รับหมายเรียกของ
        </Label>
        <Col xl={4}>
          <Input type="text" onChange={(e) => handleInputChange(e, 'conscriptFrom')} />
        </Col>
      </FormGroup>
      <FormGroup row>
        <Label className="text-end" xl={2}>
        <span className="me-1 text-danger">*</span>ที่
        </Label>
        <Col xl={4}>
          <Input type="text" onChange={(e) => handleInputChange(e, 'conscriptPlace')} />
        </Col>
        <Label className="text-end" xl={2}>
        <span className="me-1 text-danger">*</span>ลงวันที่
        </Label>
        <Col xl={3}>
        <Flatpickr
            ...
            value={data.conscriptDate}
            onChange={(e) => {
                setData((prev) => ({ ...prev, conscriptDate: e[0]}))
            }}
          />
        </Col>
      </FormGroup>
      <FormGroup row>
        <Label className="text-end" xl={2}>
        <span className="me-1 text-danger">*</span>ให้เข้ารับการ
        </Label>
        <Col xl={4}>
          <Input type="text" onChange={(e) => handleInputChange(e, 'conscriptFor')} />
        </Col>
        </FormGroup>
      <FormGroup row>
        <Label className="text-end" xl={2}>
        <span className="me-1 text-danger">*</span>ณ ที่
        </Label>
        <Col xl={4}>
          <Input type="text" onChange={(e) => handleInputChange(e, 'conscriptToPlace')} />
        </Col>
        </FormGroup>
      <FormGroup row>
        <Label className="text-end" xl={2}>
